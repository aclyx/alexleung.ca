...
describe("mandelbrot escape-time logic", () => {
  ...
  it("produces a normalized smooth-color value for escaped points", () => {
    const point = iterateMandelbrot(new Decimal("0.5"), new Decimal("0.5"), 64);
    const smoothValue = normalizedEscapeValue(point, 64);
  ...
  it("uses perturbation rendering for practical deep zoom widths", () => {
    ...
    expect(shouldUsePerturbationIteration(new Decimal("1e-40"))).toBe(true);
