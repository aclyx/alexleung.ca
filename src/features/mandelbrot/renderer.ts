...
const MAX_CPU_PIXEL_ITERATIONS = 80_000_000;
...
export async function renderMandelbrotOnCpu({
  viewport,
  size,
  settings,
  signal,
...
export async function renderMandelbrotWithStrategy(
  ...
  renderOnCpu: (
    request: RenderRequest
  ) => Promise<boolean> = renderMandelbrotOnCpu
): Promise<RenderExecutionResult> {
  const pixelCount = Math.max(
    1,
    Math.round(request.size.width) * Math.round(request.size.height)
  );
  const cpuIterationLimit = Math.max(
    25,
    Math.floor(MAX_CPU_PIXEL_ITERATIONS / pixelCount)
  );
  const effectiveCpuIterations = Math.min(
    request.settings.maxIterations,
    cpuIterationLimit
  );
  const cpuBudgetApplied =
    effectiveCpuIterations < request.settings.maxIterations;
  const cpuRequest = cpuBudgetApplied
    ? {
        ...request,
        settings: {
          ...request.settings,
          maxIterations: effectiveCpuIterations,
        },
      }
    : request;
    ...
    if (gpuAvailability.isAvailable) {
      ...
      if (gpuRenderResult.rendered) {
        return {
          ...
          effectiveMaxIterations: request.settings.maxIterations,
      ...
      if (request.signal?.aborted) {
        return {
          ...
          backend: "cpu",
          gpuFallbackReason: gpuRenderResult.fallbackReason,
          effectiveMaxIterations: effectiveCpuIterations,
          cpuBudgetApplied,
        };
      }

      const completedWithCpu = await renderOnCpu(cpuRequest);
