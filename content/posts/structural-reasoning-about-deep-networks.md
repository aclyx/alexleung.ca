...
[Chapter 6, _Deep Feedforward Networks_](https://www.deeplearningbook.org/contents/mlp.html), separates three questions that are easy to blur: what a network can represent, how efficiently it can represent it, and whether optimization can find the parameters. Architecture is not just a tuning dimension. It is a structural assumption about the function class we are willing to search.
...
The split is useful when comparing shallow and deep networks. A single hidden layer network can approximate complex structured functions, but the width required may scale exponentially for certain compositional forms. Depth can reduce parameter count by reusing intermediate computations. In that sense, depth changes scaling behavior, not just capacity.

The most useful refinement was separating topology from optimization. The architecture defines the hypothesis class; gradient descent explores it imperfectly. The fact that a function is representable says nothing about whether it is practically learnable. That separation gives me a cleaner way to evaluate model design choices.
...
This reframed low-rank adaptation for me. The architecture itself encodes a rank prior.
...
## Architecture defines the search space
